  ...
  new Strategy(opts, async (jwt_payload, done) => {
    try {
      ...
      if (adminDoc.exists) {
        ...
        console.log('Passport Debug - User document not found in either collection');
      ...
      const hasRequiredRole = storedRoles.some(role => requiredRoles.includes(role));

      if (!hasRequiredRole) {
        console.log('Passport Debug - User does not have required roles');
        return done(null, false, { message: 'User does not have required roles' });
      }
